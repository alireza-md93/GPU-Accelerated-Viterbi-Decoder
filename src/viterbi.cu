...
__device__ __forceinline__ void bmCalc(int ind, int num, float branchMetric[][4], float* coded){
	for(int i=ind+tx; i<ind+num; i+=bdx){
		...
		branchMetric[i%SHMEMWIDTH][2] =  coded[2*i] - coded[2*i+1];
		branchMetric[i%SHMEMWIDTH][3] =  coded[2*i] + coded[2*i+1];
	}   
}

template<ACS acsType>
__device__ __forceinline__ void forwardACS(int ind, trellis_t& old, trellis_t& now, path_t pathPrev[][1<<(CL-1)], float branchMetric[][4], unsigned int& allBmInd0, unsigned int& allBmInd1){}

template<>
__device__ __forceinline__ void forwardACS<ACS::RADIX2>(int ind, trellis_t& old, trellis_t& now, path_t pathPrev[][1<<(CL-1)], float branchMetric[][4], unsigned int& allBmInd0, unsigned int& allBmInd1){
	...
	if(i%(CL-1) < CL-6){
		float bm = branchMetric[i][allBmInd0&3];
...
__device__ __forceinline__ void traceback(int endStage, int dataEndInd, path_t* data, path_t pathPrev[][1<<(CL-1)]){
	if(tx == 0){
		int winnerState = 0;
...
__global__ void viterbi_core(path_t* data, float* coded, path_t* pathPrev_all) {
	...
	float (*branchMetric)[4] = (float(*)[4])sharedMemTip;
	...
	for(int i=0; i<SHFTL+SHFTR; i++)
		forwardACS<ACStype>(i, old, now, pathPrev, branchMetric, allBmInd0, allBmInd1);
	...
	for(int slide=0; slide<DECSIZE; slide+=SLIDESIZE){
		...
		traceback(ind+SLIDESIZE-1, slide+SLIDESIZE-1, data, pathPrev);
	}
}
...
int viterbi_run(float* input_d, path_t* output_d, int messageLen, float* time, ACS acsType) {
	...
	int sharedMemSize = SHMEMWIDTH * 4 * sizeof(float);
